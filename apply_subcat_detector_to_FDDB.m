function apply_subcat_detector_to_FDDB(dataDir, dataOutputDir, D)
...
DETECTOR_FILE_PREFIX = sprintf('SUBCAT_D_%d_', D);
...
for i=1:length(detector)
  detector{i}.opts.pPyramid.nOctUp=1; % Needed in FDDB
end
detectorPrefix_FDDB = fullfile(dataOutputDir, sprintf('FDDB_D_%d_', D));
detectorFile_FDDB = [detectorPrefix_FDDB 'Detector.mat'];                  
save(detectorFile_FDDB, 'detector');
...
[miss,roc,gt,dt]=acfTest('name',detectorPrefix_FDDB,...
   'imgDir',imgTestDir,...
   'gtDir',lbsTestDir,...
   'pLoad',pLoadTest,... 
   'show',1, ...
...
%   dt_i(:,7) = dt_i(:,7)-ones(size(dt_i, 1), 1);
  if NICE_VISUALISATION
    ...
    showResOpts ={'evShow',0,'gtShow',0, 'dtShow',1, 'isMulticlass', 0, 'dtLs', '--', 'lw', 2, 'cols', 'kry'}; 
