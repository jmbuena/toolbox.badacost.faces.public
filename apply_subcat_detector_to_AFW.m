function apply_subcat_detector_to_AFW(dataDir, dataOutputDir, D)
...
for i=1:length(detector)
  detector{i}.opts.pPyramid.nOctUp=0; % Not needed in AFW
end
detectorPrefix_AFW = fullfile(dataOutputDir, sprintf('AFW_D_%d_', D));
detectorFile_AFW = [detectorPrefix_AFW 'Detector.mat'];                  
save(detectorFile_AFW, 'detector');
...
[miss,roc,gt,dt]=acfTest('name',detectorPrefix_AFW,...
   'imgDir',imgTestDir,...
   'gtDir',lbsTestDir,...
   'pLoad',pLoadTest,... 
   'show',1, ...
